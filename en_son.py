...
class MapWindow(QWidget):
    def __init__(self):
        ...
        self.map = L.map(self.ui.mapWidget, options={ 'zoomControl':False , 'attributionControl':False})
        self.map.setView([37.176234977, 33.25319639], 19)
        L.tileLayer('https://api.mapbox.com/v4/mapbox.satellite/{z}/{x}/{y}@2x.png?access_token=' + str(token), {'maxNativeZoom': 19,
         'maxZoom': 21, }).addTo(self.map)
        js_code = '''
        var greenIcon = L.icon({
            iconUrl: 'C://Users//halis gürbüz//PycharmProjects//hedef_2024//png//cizgi.png',
            iconSize: [75, 90]
        });
        var marker = L.marker([37.176234977, 33.25319639], { icon: greenIcon }).addTo(map);
        marker.addTo(map);
        

        '''
        self.map.runJavaScript(js_code)
        #self.ok = L.marker([37.176234977, 33.25319639], options = {"icon": 'greenIcon'})
        #self.map.addLayer(self.ok)
    ...
    def yukle(self):
        self.flyshark.txt_yazdir(self.ui.degerYukseklik)
        self.ui.gizle()
    ...
    def ciz(self):
        self.aralik = self.ui.degerAralik
    ...
    def spinKontrol(self):
        if self.ui.degerAralik != self.aralik :
            self.flyshark.remove_lines()
            self.flyshark.remove_polygon()
            self.ciz()
